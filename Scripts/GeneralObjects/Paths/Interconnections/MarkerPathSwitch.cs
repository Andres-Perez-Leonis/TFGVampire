...
public partial class MarkerPathSwitch : Marker2D
{
	...
	*/
	public void ReciveRoutes(Dictionary<MarkerPathSwitch, Path2D> directions, MarkerPathSwitch markerObject) {
		...
		//GD.Print("Soy " + Name + " deberia encontrar a " + markerObject.Name + " por el camino " + _pathInterconectedByMe[index].Name);
		//GD.Print("El tamano de la tabla es: " + _redirectionDictionary.Count);
		foreach (MarkerPathSwitch marker in directions.Keys)
		{
			AddRoute(marker, _pathInterconectedByMe[index]);
		}
		if(_redirectionDictionary.Count == TotalMarkers - 1) MarkerSignals.EmitCommunicateMarkerSignal();
		//GD.Print("Recibo las direcciones de " + markerObject.Name);
		//if (Name == "House2") PrintRoutes();
	}
